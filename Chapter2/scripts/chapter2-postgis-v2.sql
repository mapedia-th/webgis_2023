...
SELECT pv_th FROM province;

-- SELECT queries
SELECT pv_code, pv_th, pv_en
  FROM province
  WHERE pv_th = 'พิษณุโลก';
  
 
SELECT pv_th, avg(areashape), min(areashape), max(areashape), sum(areashape)
  FROM amphoe
  GROUP BY pv_th;
  
SELECT postgis_full_version();

SELECT ap_th, pv_th FROM amphoe;
...
SELECT count(*)
FROM amphoe WHERE pv_th = 'พิษณุโลก' ;


SELECT ST_Length(geom), ST_AsText(ST_StartPoint(geom)), 
